...
if [ -z "$INTEL_OPENVINO_DIR" ]; then
    if [ -e "$DEMOS_PATH/../../setupvars.sh" ]; then
        ...
        printf "Error: Failed to set the environment variables automatically. To set the environment variables manually, run the following command:\n source <INSTALL_DIR>/bin/setupvars.sh\n where INSTALL_DIR is the OpenVINO installation directory.\n\n"
        exit 1
    fi
    if ! source "$setvars_path"; then
        printf "Unable to run $setvars_path. Please check its presence. \n\n"
        exit 1
    fi
else
